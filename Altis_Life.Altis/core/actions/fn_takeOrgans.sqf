...
if(!([false,"organcooler",1] call life_fnc_handleInv)) exitWith {hint "You dont have an organ cooler."};
if((_unit getVariable["restrained",FALSE])) exitWith {hint "Victim must first be restrained.";};//victims hands must be rape-taped
...
if(life_inv_kidney >= 3) exitWith {hint "You can't possibly need that many organs!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
